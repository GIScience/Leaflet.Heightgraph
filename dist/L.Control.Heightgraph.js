    ...
    L.Control.Heightgraph = L.Control.extend({
      ...
      */
        ...
        for (var y = 0; y < data.length; y++) {
          ...
          for (i = 0; i < data[y].features.length; i++) {
            ...
            for (var j = 0; j < coordsLength; j++) {
              ...
              if (this.options.imperial) altitude = this._toFeet(altitude); // add elevations, coordinates and point distances only once
      ...
      _showMapMarker: function _showMapMarker(ll, height, type) {
        ...
        this._mouseHeightFocusLabelTextElev.attr("x", layerPoint.x + 5).attr("y", normalizedY + 12).text(height.toFixed(1) + (this.options.imperial ? " ft" : " m")).attr("class", "tspan mouse-height-box-text");
