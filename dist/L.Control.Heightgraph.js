...
var d3Selection_1 = d3Selection.select;
var d3Selection_2 = d3Selection.selectAll;
var d3Selection_3 = d3Selection.mouse;

var d3Collection = createCommonjsModule(function (module, exports) {
...
unwrapExports(d3Array);
var d3Array_1 = d3Array.quantile;
var d3Array_2 = d3Array.min;
var d3Array_3 = d3Array.max;
...
unwrapExports(d3Format);
var d3Format_1 = d3Format.format;
...
var d3Scale = createCommonjsModule(function (module, exports) {
// https://d3js.org/d3-scale/ v2.2.2 Copyright 2019 Mike Bostock
(function (global, factory) {
 factory(exports, d3Collection, d3Array, d3Interpolate, d3Format, d3Time, d3TimeFormat) ;
}(commonjsGlobal, (function (exports,d3Collection,d3Array,d3Interpolate,d3Format,d3Time,d3TimeFormat) {
...
unwrapExports(d3Scale);
var d3Scale_1 = d3Scale.scaleOrdinal;
...
unwrapExports(d3Drag);
var d3Drag_1 = d3Drag.drag;

var d3Axis = createCommonjsModule(function (module, exports) {
...
unwrapExports(d3Axis);
var d3Axis_1 = d3Axis.axisLeft;
var d3Axis_2 = d3Axis.axisBottom;
...
var d3Shape_1 = d3Shape.curveBasis;
var d3Shape_2 = d3Shape.curveLinear;
var d3Shape_3 = d3Shape.line;
var d3Shape_4 = d3Shape.area;
var d3Shape_5 = d3Shape.symbol;
var d3Shape_6 = d3Shape.symbolTriangle;

var d3ScaleChromatic = createCommonjsModule(function (module, exports) {
// https://d3js.org/d3-scale-chromatic/ v1.5.0 Copyright 2019 Mike Bostock
(function (global, factory) {
 factory(exports, d3Interpolate, d3Color) ;
}(commonjsGlobal, function (exports, d3Interpolate, d3Color) {
...
unwrapExports(d3ScaleChromatic);
var d3ScaleChromatic_1 = d3ScaleChromatic.schemeAccent;
var d3ScaleChromatic_2 = d3ScaleChromatic.schemeDark2;
var d3ScaleChromatic_3 = d3ScaleChromatic.schemeSet2;
var d3ScaleChromatic_4 = d3ScaleChromatic.schemeCategory10;
var d3ScaleChromatic_5 = d3ScaleChromatic.schemeSet3;
var d3ScaleChromatic_6 = d3ScaleChromatic.schemePaired;
...
(function (factory, window) {
  ...
  L.Control.Heightgraph = L.Control.extend({
    ...
    onAdd: function onAdd(map) {
      ...
      var svg = this._svg = d3Selection_1(this._container).append("svg").attr("class", "heightgraph-container").attr("width", this._svgWidth + this._margin.left + this._margin.right).attr("height", this._svgHeight + this._margin.top + this._margin.bottom).append("g").attr("transform", "translate(" + this._margin.left + "," + this._margin.top + ")");
      return container;
    },
    ...
    _drawDragRectangle: function _drawDragRectangle() {
      ...
      var dragEndCoords = this._dragCurrentCoords = d3Selection_3(this._background.node());
      var x1 = Math.min(this._dragStartCoords[0], dragEndCoords[0]),
          x2 = Math.max(this._dragStartCoords[0], dragEndCoords[0]);

      if (!this._dragRectangle && !this._dragRectangleG) {
        var g = d3Selection_1(this._container).select("svg").select("g");
    ...
    _dragStartHandler: function _dragStartHandler() {
      ...
      event.stopPropagation();
      this._gotDragged = false;
      this._dragStartCoords = d3Selection_3(this._background.node());
    },
    ...
    _expand: function _expand() {
      if (!this._showState) {
        d3Selection_1(this._button).style("display", "none");
        d3Selection_1(this._container).selectAll('svg').style("display", "block");
        d3Selection_1(this._closeButton).style("display", "block");
      } else {
        d3Selection_1(this._button).style("display", "block");
        d3Selection_1(this._container).selectAll('svg').style("display", "none");
        d3Selection_1(this._closeButton).style("display", "none");
      }
    ...
    _d3ColorCategorical: [d3ScaleChromatic_1, d3ScaleChromatic_2, d3ScaleChromatic_3, d3ScaleChromatic_4, d3ScaleChromatic_5, d3ScaleChromatic_6],
    ...
    */
    ...
    _computeStats: function _computeStats() {
      var max = this._profile.maxElevation = d3Array_3(this._profile.elevations);
      var min = this._profile.minElevation = d3Array_2(this._profile.elevations);
      var quantile = this._profile.elevationQuantile = d3Array_1(this._profile.elevations, 0.75);
    ...
    _showMarker: function _showMarker(ll, height, type) {
      ...
      if (!this._mouseHeightFocus) {
        var heightG = d3Selection_1(".leaflet-overlay-pane svg").append("g");
      ...
      d3Selection_2('.bBox').attr("width", maxWidth + 10).attr("height", maxHeight);
    },
    ...
    _createFocus: function _createFocus() {
      ...
      d3Selection_2('.focusbox rect').attr("height", height * textDistance + textDistance / 2).attr("display", "block");
    ...
    _createHorizontalLine: function _createHorizontalLine() {
      ...
      var dragstart = function dragstart(d) {
        d3Selection_1(this).raise().classed("active", true);
        d3Selection_1(".horizontalLine").raise().classed("active", true);
      };

      var dragged = function dragged(d) {
        var maxY = self._svgHeight;
        var eventY = d3Selection_3(self._container)[1] - 10;
        d3Selection_1(this).attr("transform", function (d) {
          return "translate(" + d.x + "," + (eventY < 0 ? 0 : eventY > maxY ? maxY : eventY) + ") rotate(" + d.angle + ")";
        });
        d3Selection_1(".horizontalLine").attr("y1", eventY < 0 ? 0 : eventY > maxY ? maxY : eventY).attr("y2", eventY < 0 ? 0 : eventY > maxY ? maxY : eventY);
        ...
        d3Selection_1(".horizontalLineText").attr("y", eventY <= 10 ? 0 : eventY > maxY ? maxY - 10 : eventY - 10).text(d3Format_1(".0f")(self._y.invert(eventY < 0 ? 0 : eventY > maxY ? maxY : eventY)) + " m");
      ...
      var dragend = function dragend(d) {
        d3Selection_1(this).classed("active", false);
        d3Selection_1(".horizontalLine").classed("active", false);
      ...
      var horizontalDrag = this._svg.selectAll(".horizontal-symbol").data(jsonTriangle).enter().append("path").attr("class", "lineSelection").attr("d", d3Shape_5().type(function (d) {
        return d.type;
      }).size(function (d) {
        return d.size;
        ...
        return d.color;
      }).call(d3Drag_1().on("start", dragstart).on("drag", dragged).on("end", dragend));
    },
    ...
    _appendScales: function _appendScales() {
      ...
      this._x = d3Scale_2().range([0, width]);
      this._y = d3Scale_2().range([height, 0]);
      ...
      if (shortDist === true) {
        this._xAxis = d3Axis_2().scale(this._x).tickFormat(function (d) {
          return d3Format_1(".2f")(d) + " km";
        });
      } else {
        this._xAxis = d3Axis_2().scale(this._x).tickFormat(function (d) {
          return d3Format_1(".0f")(d) + " km";
        });
      }
      ...
      this._yAxis = d3Axis_1().scale(this._y).tickFormat(function (d) {
        return d + " m";
      });
      ...
      this._yEndAxis = d3Axis_3().scale(this._yEnd).ticks(0);
    },
    ...
    _appendAreas: function _appendAreas(block, idx, eleIdx) {
      ...
      var self = this;
      var area = this._area = d3Shape_4().x(function (d) {
    ...
    _createSelectionBox: function _createSelectionBox() {
      var self = this;
      var svg = d3Selection_1(this._container).select("svg");
      var margin = this._margin,
          width = this._width - this._margin.left - this._margin.right,
          height = this._height - this._margin.top - this._margin.bottom;
      var jsonTriangles = [{
      ...
      var selectionSign = svg.selectAll(".select-symbol").data(jsonTriangles).enter().append("path").attr("class", "select-symbol").attr("d", d3Shape_5().type(function (d) {
        return d.type;
      })).attr("transform", function (d) {
        return "translate(" + d.x + "," + d.y + ") rotate(" + d.angle + ")";
      }).attr("id", function (d) {
    ...
    _createLegend: function _createLegend() {
      ...
      legendHover.append('text').attr('class', 'legend-menu').attr("class", "no-select").attr('x', 15).attr('y', height + 40).text(function (d, i) {
        ...
        d3Selection_1('.legend-box').style("display", "block");
        d3Selection_2('.legend').style("display", "block");
      }).on('mouseleave', function () {
        d3Selection_1('.legend-box').style("display", "none");
        d3Selection_2('.legend').style("display", "none");
      });
    },
    ...
    */
    _dynamicBoxSize: function _dynamicBoxSize(className) {
      var cnt = d3Selection_2(className).nodes().length;
      var widths = [];

      for (var i = 0; i < cnt; i++) {
        widths.push(d3Selection_2(className).nodes()[i].getBoundingClientRect().width);
      }

      var maxWidth = d3Array_3(widths);
      return [cnt, maxWidth];
    },
    ...
    _createBorderTopLine: function _createBorderTopLine() {
      ...
      var borderTopLine = d3Shape_3().x(function (d) {
        var x = self._x;
        return x(d.position);
      }).y(function (d) {
        var y = self._y;
    ...
    _mousemoveHandler: function _mousemoveHandler(d, i, ctx) {
      var coords = d3Selection_3(this._svg.node());
      var areaLength;
    ...
    _findItemForX: function _findItemForX(x) {
      var bisect = d3Array_4(function (d) {
        return d.position;
      }).left;
